        ...
        <ul className="grid gap-2 grid-cols-2">
          {slots.map((s) => (
            <li key={`${s.staffId}-${s.start}`}>
              <Link aria-label={`Select ${new Date(s.start).toLocaleTimeString()}`} className="block border rounded p-2 hover:bg-gray-50" prefetch href={`/book/details?serviceId=${serviceId}&staffId=${s.staffId}&start=${encodeURIComponent(s.start)}`}>{new Date(s.start).toLocaleTimeString()}</Link>
            </li>
          ))}
        </ul>
      )}
